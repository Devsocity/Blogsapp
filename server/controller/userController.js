...
export const fetchSingleUser = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({ data: user, message: 'User get success.' });
});
...
export const deleteUser = asyncHandler(async (req, res) => {
  ...
  res.status(200).clearCookie('aToken').json({ data: user, message: 'Account deleted successful.' });
});
...
export const updateUser = asyncHandler(async (req, res) => {
  ...
  if (!req?.body) return res.status(400).json({ message: 'You have nothing changed' });
